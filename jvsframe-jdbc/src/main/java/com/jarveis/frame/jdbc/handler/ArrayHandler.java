package com.jarveis.frame.jdbc.handler;

import java.sql.ResultSet;
import java.sql.ResultSetMetaData;
import java.sql.SQLException;
import java.sql.Types;

import com.jarveis.frame.jdbc.ResultSetHandler;

/**
 * 数组对象结果集
 * 
 * @author liuguojun
 */
public class ArrayHandler implements ResultSetHandler {

	public Object handle(ResultSet rs) throws SQLException {

		Object[] result = null;
		ResultSetMetaData meta = rs.getMetaData();
		int cols = meta.getColumnCount();
		if (rs.next()) {
			result = new Object[cols];
			for (int i = 0; i < cols; i++) {
				String label = meta.getColumnLabel(i + 1);
				int type = meta.getColumnType(i + 1);
				
				switch (type) {
				case Types.CHAR:
				case Types.VARCHAR:
				case Types.LONGVARCHAR:
					result[i] = rs.getString(label);
					break;
				case Types.TINYINT:	
				case Types.SMALLINT:
				case Types.INTEGER:
					result[i] = rs.getInt(label);
					break;
				case Types.BIGINT:
					result[i] = rs.getLong(label);
					break;
				case Types.FLOAT:
					result[i] = rs.getLong(label);
					break;
				case Types.REAL:
				case Types.DOUBLE:
					result[i] = rs.getDouble(label);
					break;
				case Types.DECIMAL:
				case Types.NUMERIC:
					result[i] = rs.getBigDecimal(label);
					break;
				case Types.DATE:
				case Types.TIME:
				case Types.TIMESTAMP:
					result[i] = rs.getTimestamp(label);
					break;
				default:
					result[i] = rs.getObject(label);
				}
			}
		}

		return result;
	}

}
