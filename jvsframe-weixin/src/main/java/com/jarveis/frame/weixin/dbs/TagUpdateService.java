 ...
 *
 * @author liuguojun
 * @see WeixinURL#UPDATE_TAG
 * @since 2021-10-18
 */
public class TagUpdateService implements Service, WeixinCallback, WeixinAccessToken {
    ...
    public Param callService(Param in) {
        ...
        try {
            ...
            String json = "{ \"tag\":{ \"id\":%s, \"name\":\"%s\" } }";

            // 获取accessToken
            String accessToken = getAccessToken(in);

            // 请求接口
            String postURL = String.format(WeixinURL.UPDATE_TAG, accessToken);
            String postJson = String.format(json, id, name);
