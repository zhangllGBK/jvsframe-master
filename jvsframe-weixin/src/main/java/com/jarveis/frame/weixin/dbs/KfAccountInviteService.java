 ...
 *
 * @author liuguojun
 * @see WeixinURL#INVITE_CUSTOM_KFACCOUNT
 * @since 2021-06-16
 */
public class KfAccountInviteService implements Service, WeixinCallback, WeixinAccessToken {

    private static final Logger log = LoggerFactory.getLogger(KfAccountInviteService.class);
    ...
    public Param callService(Param in) {
        ...
        try {
            ...
            String invite_wx = in.getBody().getString("@invite_wx"); // 邀请的微信账户
            if (StringUtils.isAnyEmpty(kf_account, invite_wx)) {
            ...
            String json = "{\"kf_account\" : \"%s\",\"invite_wx\" : \"%s\"}";

            // 请求接口
            String postURL = String.format(WeixinURL.INVITE_CUSTOM_KFACCOUNT, accessToken);
            // 请求参数
            String postJson = String.format(json, kf_account, invite_wx);
