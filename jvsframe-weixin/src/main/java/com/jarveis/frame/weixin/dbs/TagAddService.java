 ...
 *
 * @author liuguojun
 * @see WeixinURL#ADD_TAG
 * @since 2021-10-18
 */
public class TagAddService implements Service, WeixinCallback, WeixinAccessToken {

    private static final Logger log = LoggerFactory.getLogger(TagAddService.class);
    ...
    public Param callService(Param in) {
        ...
        try {
            ...
            String json = "{ \"tag\":{ \"name\":\"%s\" } }";

            // 获取accessToken
            String accessToken = getAccessToken(in);

            // 请求接口
            String postURL = String.format(WeixinURL.ADD_TAG, accessToken);
            String postJson = String.format(json, name);
