 ...
 *
 * @author liuguojun
 * @see WeixinURL#LIST_USER_TAG
 * @since 2021-10-18
 */
public class UserTagQueryService implements Service, WeixinCallback, WeixinAccessToken {

    private static final Logger log = LoggerFactory.getLogger(UserTagQueryService.class);
    ...
    public Param callService(Param in) {
        ...
        try {
            ...
            String json = "{ \"openid\":\"%s\" }";

            // 获取accessToken
            String accessToken = getAccessToken(in);

            // 请求接口
            String postURL = String.format(WeixinURL.LIST_USER_TAG, accessToken);
            String postJson = String.format(json, openid);
