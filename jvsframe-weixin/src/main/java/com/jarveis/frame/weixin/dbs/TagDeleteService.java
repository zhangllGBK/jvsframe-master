 ...
 * <pre>
 *     接口文档：
 *     <a href="https://developers.weixin.qq.com/doc/offiaccount/User_Management/User_Tag_Management.html">...</a>
 *
 *     输出：
 *     <Resp>
 *         <head errcode="0000"/>
 ...
 *
 * @author liuguojun
 * @see WeixinURL#DELETE_TAG
 * @since 2021-10-18
 */
public class TagDeleteService implements Service, WeixinCallback, WeixinAccessToken {

    private static final Logger log = LoggerFactory.getLogger(TagDeleteService.class);
    ...
    public Param callService(Param in) {
        ...
        try {
            ...
            String id = in.getBody().getString("@id");
            if (StringUtils.isEmpty(id)) {
                throw new Exception("标签Id不能为空");
            }

            String json = "{ \"tag\":{ \"id\":%s } }";

            // 获取accessToken
            String accessToken = getAccessToken(in);

            // 请求接口
            String postURL = String.format(WeixinURL.DELETE_TAG, accessToken);
            String postJson = String.format(json, id);
