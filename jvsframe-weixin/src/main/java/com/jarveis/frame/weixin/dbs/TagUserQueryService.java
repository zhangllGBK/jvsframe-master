 ...
 *
 * @author liuguojun
 * @see WeixinURL#LIST_TAG_USER
 * @since 2021-10-18
 */
public class TagUserQueryService implements Service, WeixinCallback, WeixinAccessToken {

    private static final Logger log = LoggerFactory.getLogger(TagUserQueryService.class);
    ...
    public Param callService(Param in) {
        ...
        try {
            ...
            String json = "{ \"tagid\":%s, \"next_openid\":\"%s\" }";

            // 获取accessToken
            String accessToken = getAccessToken(in);

            // 请求接口
            String postURL = String.format(WeixinURL.LIST_TAG_USER, accessToken);
            String postJson = String.format(json, tagid, nextOpenid);
